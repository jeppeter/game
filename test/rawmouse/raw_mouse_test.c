...
MainWndProc (HWND hwnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
        ...
        case WM_DESTROY:
		  ...
		  for (i = 0; i < raw_mouse_count(); i++) {
		    ...
		    DrawText(hdc, cBuf, wcslen(cBuf), &rc, DT_CENTER);
...
WinMain (HINSTANCE hInst, HINSTANCE hPrev, LPSTR lpCmd, int nShow)
{
        ...
        wchar_t*        szMainWndClass = TEXT("WinTestWin");
	...
	if (!init_raw_mouse(1, 0, 1)) { // registers for (sysmouse=yes,  terminal services mouse=no, HID_mice=yes)
	  MessageBox(NULL, TEXT("RawInput not supported by Operating System.  Exiting.") , TEXT("Error!") ,MB_OK);
	  return 0; 
	}
